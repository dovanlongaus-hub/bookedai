...
- Backend AI routing defaults to OpenAI (`OPENAI_API_KEY`, `OPENAI_MODEL=gpt-5-mini`) and can fall back to Claude through Anthropic auth (`ANTHROPIC_API_KEY`, `ANTHROPIC_MODEL`) for structured AI replies; internet/public-web service discovery remains OpenAI-backed through Responses `web_search` and labels sourced external options as `public_web_search`.
...
- Latest local Product search recovery: warning-only live-read responses no longer clear the product chat shortlist when there are instant catalog or legacy chat matches; the assistant keeps closest catalog matches visible with fallback copy, and product regression now covers this Android-sized mobile branch. Production deploy is still pending.
...
- Latest homepage chat-search recovery: failed live search/matching/chat requests now recover with fast preview/catalog fallback results and customer-safe guidance instead of showing raw loading/backend failure text; frontend build, homepage responsive Playwright, live deploy, stack health, and production browser smoke passed on `2026-04-29`.
- Latest homepage/product search relevance closeout: `bookedai.au` explicit intent+location searches suppress unrelated results, long homepage shortlists show the first `3` cards with `Search more`, and the assistant pane has a tested scroll container. `product.bookedai.au` now filters rendered fallback cards by active intent/location, uses exact-word short-token matching, and falls back to the unscoped BookedAI catalog only when the product-scoped catalog has no relevant match. Live deploy and production UAT for chess/Sydney plus Future Swim/Miranda no-noise checks passed on `2026-04-29`.
- Latest homepage location-search update: `bookedai.au` now asks for device location just in time when the query is near-me/location-sensitive, showing `Use current location` and `Type suburb instead` choices in chat. Browser geolocation is only requested after user approval; denied/unavailable location keeps stale or broad fallback cards hidden and asks for a suburb/area. Local frontend build and focused live-read Playwright JIT/relevance/top-3 coverage passed on `2026-04-29`.
- Latest homepage booking-layout update: the `bookedai.au` assistant keeps the active selected match, booking form, or confirmation above progress/follow-up/guidance panels, and the chat/results pane is a viewport-bounded scroll surface with a visible scrollbar. Local frontend build and focused live-read Playwright pagination/scroll/order coverage passed on `2026-04-29`.
- Latest homepage winning-SME UX update: `bookedai.au` now leads with winning more bookings from existing enquiries, shows fuller result decision details before booking, keeps `info@bookedai.au` visible as the public contact path, and scopes the Booking Activity drawer to tenant ref `bookedai-au` with customer-safe tenant-session error handling. Local frontend build, homepage responsive/UI Playwright (`5 passed`), diff check, live deploy, stack health, and production browser smoke passed on `2026-04-29`; broader live-read booking-submit fixture drift remains a separate follow-up.
- Latest local tenant workspace UX update: `tenant.bookedai.au` has an enterprise console shell with sticky top bar, icon actions, a 2xl desktop icon rail, sticky left menu, right content frame, contextual panel actions, and a fixed mobile bottom icon nav. Follow-up QA fixed the 1280px squeeze and mobile bottom-bar layout; TypeScript, frontend build, tenant gateway/workspace Playwright (`6 passed`), and tenant smoke (`6 passed`) passed locally on `2026-04-29`.
- Latest local Product booking-form cleanup: `product.bookedai.au` now removes the verbose pre-submit `Your booking journey` guidance card and duplicate desktop journey stepper, keeping the form focused on selected service, contact details, preferred time, and confirmation; build/diff passed and the affected mobile/desktop product full-flow tests passed, with one unrelated preview bootstrap smoke still failing under port `3100` contention.
- Latest public booking confirmation follow-up: `bookedai-au` public bookings now send a compact Twilio SMS confirmation in addition to WhatsApp when a phone is present, and AI Mentor service lookups avoid invalid uuid casts against varchar tenant fields. `py_compile`, diff check, frontend build, deploy-live, stack health, and production homepage smoke passed on `2026-04-29`.
