...
- Latest AI Mentor tenant closeout: `ai-mentor-doer` login/contact is `aimentor@bookedai.au / FirstHundred1M$` with contact phone `+84908444095`; migration `023` is applied live; `main` is at deploy `3f8bcb9`; release gate and live deploy passed on `2026-04-28`; AI Mentor embed URL and widget asset return `200`, while target CORS `OPTIONS 204` remains a follow-up because the static asset currently returns `405`.
