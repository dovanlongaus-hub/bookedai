...
import { bookingRescheduleSchema, bookingRouteIdSchema } from "@/lib/validation/admin";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const payload = bookingRescheduleSchema.parse(await request.json());
    const booking = await repository.rescheduleBooking(
      tenant.tenantId,
      bookingId,
      payload.startAt,
      payload.endAt,
