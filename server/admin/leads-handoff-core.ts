...
export type LeadsHandoffGuardState = {
  tenantMismatch: boolean;
  handoffRequiresReentry: boolean;
  leadsReadOnly: boolean;
};

export function parseLeadsHandoff(
  searchParams:
  ...
  const requestedTenantId = normalizeToken(getValue('tenantId') ?? getValue('expectedTenantId'));
  const requestedTenantSlug = normalizeToken(getValue('tenantSlug') ?? getValue('expectedTenantSlug'));
...
export function buildLeadsHandoffReturnHref(state: LeadsHandoffState) {
...
export function getLeadsHandoffGuardState(input: {
  handoff: LeadsHandoffState;
  resolvedTenantId: string;
  resolvedTenantSlug: string;
  supportModeActive: boolean;
}): LeadsHandoffGuardState {
  ...
  return {
    ...
    leadsReadOnly: input.supportModeActive || tenantMismatch || handoffRequiresReentry,
  };
}
