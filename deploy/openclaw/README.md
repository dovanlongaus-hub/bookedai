...
- The `openclaw-cli` service is rootless by default (`OPENCLAW_CLI_USER=1000:1000`), mounts only the OpenClaw config/workspace, the BookedAI repo, and Codex home, and does not mount `/`, `/hostfs`, or `/var/run/docker.sock`.
- The default Telegram/OpenClaw action vocabulary no longer grants `host_shell`, `openclaw_runtime_admin`, or `full_project`. Operators can still use repo-scoped commands, read-only status checks, and allowlisted `host-command` programs.
- `python3 scripts/telegram_workspace_ops.py host-shell --command "..." --cwd /` is now a break-glass lane only. It is blocked unless `BOOKEDAI_ENABLE_HOST_SHELL=1` is set for an explicit operator session, and it should not be part of the default OpenClaw compose runtime.
...
- For trusted full control from `bot.bookedai.au` webchat and Telegram, `python3 scripts/telegram_workspace_ops.py enable-openclaw-full-access` remains a break-glass repair tool only. It must be paired with an explicit temporary action allowlist and `BOOKEDAI_ENABLE_HOST_SHELL=1` when host shell is truly required; add `--telegram-open` only if every Telegram sender should be allowed to control the bot.
