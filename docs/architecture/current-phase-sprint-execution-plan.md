...
- `2026-04-29` tenant enterprise workspace shell is closed locally for the tenant surface: `tenant.bookedai.au` now has a sticky enterprise top bar, icon actions, 2xl desktop icon rail, sticky left menu, right content frame, contextual panel actions, and mobile bottom icon navigation while preserving existing tenant read/write logic. TypeScript, frontend build, tenant smoke (`6 passed`), and tenant Playwright UAT/A-B (`6 passed`) verified gateway continuity, mocked workspace navigation, command palette, random panel switching across all panels, control/revenue-ops variant stability, screenshots, and no horizontal overflow.
...
- `2026-04-28` product UI/auth/search UX review is closed locally for Phase 17 stabilization: the product header now removes redundant logo-side text, adds a compact Google/email account menu, stores a local customer booking profile for repeat booking prefill, hides the flow strip on mobile, and keeps the desktop/tablet journey status-aware across `Chat -> Search -> Preview -> Book -> Pay -> Care`; instant tenant/catalog matches and timeout-bounded live-read progress improve slow-search perception. Local verification passed with Vite build, product regression Playwright (`7 passed`), and responsive no-overflow sweeps at `390`, `412`, and `1440` widths. Full npm build is blocked by unrelated ChessGrandmaster TypeScript drift.
- `2026-04-28` product mobile shell compacting is closed locally for Phase 17 UI/UX stabilization: the product first-screen explainer was shortened, the redundant mobile trial sentence was removed, flow steps became compact horizontal chips, the duplicate mobile pilot CTA was hidden, and the assistant frame now receives the freed vertical space; frontend build and product regression Playwright (`6 passed`) verified no horizontal overflow regression.
- `2026-04-28` communication template polish is closed locally for Phase 19: outbound confirmation email now uses the official BookedAI.au logo, a dark brand header, status badge, structured summary, CTA, and BookedAI Manager Bot support footer; WhatsApp lifecycle/booking confirmations use native short-line `*bold*` formatting; Telegram search/order/booking replies use compact HTML-safe status/action sections with inline controls.
...
- `2026-04-27` public/pitch video replacement has been refreshed again: `bookedai.au` and `pitch.bookedai.au` now point their native pitch video embeds and direct video links to `https://upload.bookedai.au/videos/9eb8/BhVuOlB2QXlBo-_nyOFCcA.mp4`, with direct byte-range media verification returning `206 video/mp4`.
...
- `2026-04-27` product full-flow UI/UX merge has been applied locally: `product.bookedai.au` now presents `Chat request -> Search -> Preview -> Select -> Book -> Payment posture -> Thank You / Portal -> Add calendar -> CRM/email/messaging -> Telegram/customer-care handoff` as one status-aware journey, with confirmation cards anchored to the booking reference and `BookedAI Manager Bot / portal` care handoff rather than overclaiming outbound Telegram completion.
- `2026-04-27` Phase 19 Telegram continuity follow-up is now active locally: public/product booking surfaces can hand off into `@BookedAI_Manager_Bot` with service-aware or booking-aware context, `/start bk.<booking_reference>` reopens booking-aware customer care, admin Reliability/Integrations now surfaces protected customer-agent health plus Telegram reply/callback, webhook backlog, identity watchlist, and tenant CRM/email/notify spotlight data, and the remaining gate is backend pytest enablement plus live `chess` UAT evidence.
- `2026-04-27` Telegram existing-booking care now keeps support-escalation detection scoped to the latest customer turn, preventing prior assistant history like `Support contact` from turning neutral payment/status questions into queued support requests; care replies also expose booking portal and QR inline controls.
- `2026-04-27` portal QR/payment/channel clarity is closed locally: `portal.bookedai.au` now shows booking portal QR, payment QR/posture, copy/save/download controls, canonical booking-reference URL, Telegram/WhatsApp continuation links with the ID attached, and visible action-effect guidance for pay, reschedule/change, cancel, help, and add-new-booking paths.
...
- BookedAI email sender identity is now standardized on `info@bookedai.au`; the backend defaults `EMAIL_SMTP_USERNAME` and `EMAIL_SMTP_FROM` to that address, blank email env values fall back to the same mailbox, Docker dev/prod compose defaults no longer inject empty SMTP/From identities, and bookedai.au portal/support fallbacks use the same address for customer-visible email continuity
...
- `2026-04-26` Sprint 19 P0-8 is closed live: `openclaw-cli` runs as `OPENCLAW_CLI_USER=1000:1000`, no longer mounts hostfs or Docker socket by default, default Telegram actions exclude `host_shell`, `openclaw_runtime_admin`, and `full_project`, `host-shell` requires `BOOKEDAI_ENABLE_HOST_SHELL=1`, the live OpenClaw stack was recreated, gateway health passed, and operator smoke verified the reduced authority.
- `2026-04-26` Phase 17 P1-8 pitch coverage closed locally: `frontend/tests/pitch-deck-rendering.spec.ts` now covers the pitch deck at `1440px` desktop and `390px` mobile, verifies key investor/proof sections and the refreshed hosted video source, and enforces no horizontal overflow. The earlier P1-7 product regression rerun follow-up also passed locally.
- `2026-04-26` Sprint 19/20 carry-forward closeout: P0-5 tenant validator is closed live after a production tenant-mismatch smoke returned `403`; P1-9 Future Swim Miranda URL hotfix is closed live after the catalog row moved from the dead `/locations/miranda/` URL to the live `/locations/` URL; P0-4 now has its missing `022` evidence-index migration in the repo and applied live, with Telegram customer UAT/evidence drawer still carried.
- `2026-04-26` Phase 19 P1-3 is closed locally: WhatsApp webhook tests now cover identity-gate, queued cancel, queued reschedule, and Internet expansion at Telegram parity. FX-2 shared API timeout first pass is closed live through `frontend/src/shared/api/client.ts`; direct fetch cleanup remains carried. Phase 17 FX-7 portal URL canonicalization is closed live with canonical `booking_reference` rewrite behavior, Playwright coverage for camelCase/hash/conflict links, release-gate pass, deploy-live, stack health, and live `portal.bookedai.au` smoke.
...
1. `UI/UX stabilization`
   ...
   - public chat lead capture must stay on tenant-scoped public endpoints and never expose raw tenant-session validation errors to customers
...
- close P0-1 (`portal v1-* snapshot 500`), P0-2 (`WhatsApp provider posture`), P0-3 (`Telegram secret-token + Evolution HMAC`), P0-4 (`webhook idempotency`; code/indexes live, Telegram UAT/evidence drawer carried), P0-5 (`actor_context.tenant_id validator`; closed live), P0-6 (`GitHub Actions CI`; blocked by token scope), P0-7 (`.env.production.example`), P0-8 (`OpenClaw root drop`; closed live)
...
- close P1-4 (`tenant_app_service split`), P1-6 (`beta DB separation + image registry`), P1-10 (`channel-aware email templates`); P1-3 (`WhatsApp test parity with Telegram`) is closed locally and P1-8 (`pitch + 390px Playwright`) has local coverage awaiting promotion
- carry P1-11 (`Telegram deep-link continuity`) and P1-12 (`admin/customer-care observability`) from local implementation into live UAT evidence and direct backend pytest reruns before declaring the wider Phase 19 care loop stable
