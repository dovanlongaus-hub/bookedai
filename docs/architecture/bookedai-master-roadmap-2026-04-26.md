...
`2026-05-01`: the AI Mentor sub-project end-to-end build-out has shipped. Four PRs (#26 Phases 1-4 + cleanup, #27 Phase 3 deeper, #30 chat/turn + Piper TTS scaffold, #31 bot voice-out wiring) landed across `2026-04-30` and `2026-05-01`. Capabilities now live on `aimentor.bookedai.au`: booking cancel/reschedule end-to-end through web + Telegram + WhatsApp with Zoho Meeting cancel/update + Zoho CRM Deal stage sync + lifecycle email; programs catalog API replacing the hardcoded frontend list; voice mode toggle on the web chat with male teacher-tone speech-synthesis (Daniel/Aaron/David priority) and dedup-by-message-id watcher; daily retention worker dispatching T+1d / T+7d / T+30d / T+90d touches with channel priority `last_engaged_channel`; status_update orchestrator mapping completed/no-show/refunded/in-progress to Zoho Deal stages and triggering completion thank-you email; LLMRouter (OpenAI 5.4 primary, Claude Sonnet via taphoaapi fallback, both via the `openai` SDK with different `base_url`); Telegram inline-keyboard direct booking (`/programs` → program → slot → name/email/phone → YES/NO confirm); chat/turn endpoint backed by LLMRouter with 5-min catalog cache + 60/min rate limit + hallucination-guarded program-id filter + strict-JSON contract; Whisper STT for Telegram voice notes; Piper TTS scaffold (`rhasspy/wyoming-piper` Docker, voice `en_US-ryan-medium`); bot voice-out wiring sending OGG via Telegram `sendVoice` (feature-flagged `ENABLE_BOT_VOICE_OUT=true`, default off). Account portal Progress tab visualizes booking journey timeline. 14 lifecycle email templates EN+VI shipped (cancellation, reschedule confirmation, T1, T7, T30 promo, winback, completion thank-you, plus the existing welcome + monthly check-in). ~85 backend tests + 9 Playwright tests across the four merged PRs. Single source of truth: `docs/architecture/aimentor-product-requirements-2026-05-01.md`. Sub-project pattern documented as PRD section 25 and as the AIMentor sibling section to the Chess-first priority rule in the implementation phase roadmap. Deferred to next-session work: `ConversationEngine` channel-agnostic refactor (~5 engineer-days) and WhatsApp full interactive list / template parity (Meta approval gate).

`2026-04-29`: AI model routing has been corrected to match the customer-facing booking-agent posture. Backend AI calls now default to OpenAI (`OPENAI_API_KEY`, `OPENAI_MODEL=gpt-5-mini`), can fall back to Claude through Anthropic auth (`ANTHROPIC_API_KEY`, `ANTHROPIC_MODEL`) for structured AI replies, and keep internet/public-web service discovery on OpenAI because that path uses the Responses `web_search` tool. The OpenClaw customer-agent manifest, compose env forwarding, `.env.example`, README, project index, and implementation-progress notes were updated to match this baseline.

`2026-04-28`: the Zoho CRM/Calendar payment-lifecycle setup is activated and live-deployed for Phase 21. The repo OAuth helper now requests CRM, Notifications, Calendar list/event creation, and Zoho Meeting scopes in one consent flow, and `list-calendars` selected the AU `info` calendar UID. Booking meeting provisioning now uses `ZOHO_CALENDAR_API_BASE_URL=https://calendar.zoho.com.au/api/v1` correctly, sends Zoho Calendar `eventdata`, and prefers refresh-token exchange over stale direct Calendar access tokens. Paid-booking reconciliation now attempts immediate Zoho `Deal` Closed Won plus follow-up `Task` sync with payment amount/source/time, while local audit/outbox/email remains the source of operational truth. CRM metadata smoke, Calendar list smoke, live Zoho Calendar/Meeting event smoke, deploy-live, stack health, API health, `chess.bookedai.au` shell smoke, and final live booking/payment UAT passed. Final UAT booking `v1-364c66e949` returned Zoho Meeting plus Calendar event URLs; booking `deal`/`task` and payment `deal_payment`/`task_payment` rows all synced with external Zoho ids after one throttle retry.

`2026-04-28`: `aimentor.bookedai.au` is live as the tenant-branded AI Mentor 1-1 Pro runtime for `ai-mentor-doer`, with Nginx routing, Cloudflare DNS automation, certbot deployment defaults, frontend same-origin API resolution, and backend CORS/TrustedHost defaults aligned. Live verification passed for stack health, Nginx config, homepage, account route, backend health, certificate SAN, and app-bundle routing.

`2026-04-28`: the local payment reconciliation lifecycle slice is implemented for Phase 21. A shared `PaymentLifecycleService.mark_booking_paid(...)` now owns the trusted paid-booking transition; Stripe paid webhooks call it after the existing payment mirror sync, tenant admin/finance users can confirm received QR/bank/manual payments through `POST /api/v1/payments/manual-confirm`, and return surfaces can query `GET /api/v1/payments/status` for backend truth. Paid transitions update `booking_intents.payment_dependency_state='paid'`, write payment metadata, audit/enqueue `booking.payment_paid`, and create queued CRM/email lifecycle evidence. Backend regression passed across booking, Stripe, search, messaging, and Zoho feedback lanes (`139 passed`). The closeout was published to Notion/Discord at `docs/development/telegram-sync/2026-04-28/152225-bookedai-payment-reconciliation-lifecycle-local-implementation.md`. Live deploy, portal/public status wiring, and real outbox worker dispatch remain follow-ups.

`2026-04-28`: the local Stripe best-practice integration slice is implemented for Phase 20.5/21. Direct backend Checkout/Billing calls now pin `Stripe-Version` to `2026-02-25.clover`, public booking/chat/chess/tenant subscription/pricing checkout creation no longer forces `payment_method_types[]=card`, and `scripts/stripe_sandbox_smoke.py` provides a safe sandbox-only Checkout smoke path. A follow-up review tightened homepage Stripe return handling so `booking=success` verifies `GET /api/v1/payments/status` before showing paid, added regression guards for API-version/Dynamic Payment Methods, added Playwright `verifying -> paid` coverage, and passed a real Stripe test-mode Checkout smoke. `scripts/stripe_webhook_replay_smoke.sh` plus `docs/development/stripe-cli-webhook-replay-runbook.md` now define the Stripe CLI replay path; actual replay is blocked locally because `stripe` is not installed. The operator contract is documented in `docs/development/stripe-best-practices-integration.md` and `docs/development/stripe-payment-flow-review-2026-04-28.md`; Notion/Discord archive entries are `docs/development/telegram-sync/2026-04-28/154445-bookedai-stripe-best-practices-integration.md` and `docs/development/telegram-sync/2026-04-28/155756-bookedai-stripe-payment-flow-review-and-verified-return-follow-up.md`; live deploy and Stripe CLI webhook replay remain follow-ups.

`2026-04-28`: communication template polish is locally complete for the Phase 19 customer-care layer. Confirmation email now uses the official BookedAI.au logo, dark brand header, status badge, structured booking summary, CTA, and BookedAI Manager Bot support footer. WhatsApp booking/lifecycle copy now follows native short-line `*bold*` formatting, and Telegram search/order/booking replies add compact HTML-safe status/action sections while preserving inline controls. Focused backend regression passed across lifecycle email, release-gate security, booking routes, Telegram webhook routes, and WhatsApp webhook routes; closeout was published to Notion/Discord at `docs/development/telegram-sync/2026-04-28/143344-bookedai-communication-template-polish.md`.
...
- local payment reconciliation foundation now exists: trusted Stripe/manual payment evidence can mark booking intents paid through a shared service and expose backend payment status for return surfaces; portal/public UI wiring and bank-feed automation remain next steps
...
- `2026-04-29` product search/booking execution plan: added `docs/development/product-search-booking-execution-plan-2026-04-29.md` to map product search trust, booking UI states, after-booking order detail, customer identity, wallet pass generation, QA gates, and customer-care continuity into roles, skills, phases, and acceptance gates.
- `2026-04-29` product QA/UAT deep pass: product local build/regression, after-booking responsive subset, product live-read popup tests, standard live-read smoke, focused backend booking/care/payment/messaging/calendar tests, and live `product.bookedai.au` responsive smoke passed; Playwright preview readiness timeout was extended to reduce false cold-build failures; broad homepage live-read contract drift remains a non-product follow-up.
- `2026-04-29` product after-booking order-detail UX: redesigned the confirmation state into a professional order screen with order summary, view details, portal/QR access, payment, Apple Wallet and Google Wallet entry points, print/share/email actions, and responsive mobile/Android/desktop regression coverage (`8 passed`).
- `2026-04-28` product UI/auth/search UX review: removed redundant logo-side brand text, added a compact Google/email account menu with local customer profile reuse, hid the flow strip on mobile, redesigned the desktop/tablet journey around `Chat -> Search -> Preview -> Book -> Pay -> Care`, improved slow live-read progress and tenant/catalog shortcut matching, and verified local product regression (`7 passed`) plus responsive no-overflow sweeps at `390`, `412`, and `1440` widths.
- `2026-04-28` BookedAI.au public chat tenant-lead fix: homepage/public assistant lead capture now uses `/api/v1/public/leads/{tenant_slug}` for public-web and embedded-widget runtimes with `tenantRef`; raw tenant-session errors are replaced with customer-safe recovery UI; live deploy and `bookedai-au` public lead smoke passed.
- `2026-04-28` product mobile shell compacting: shortened the product first-screen explainer, removed the redundant mobile trial sentence, compressed the flow strip into chips, gave the assistant frame more vertical space, and verified frontend build plus product regression Playwright (`6 passed`).
