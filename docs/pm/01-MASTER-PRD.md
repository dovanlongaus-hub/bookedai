...
| `FR-106` | **Customer-comms default opt-out**: any *recurring* / *automated* customer-facing dispatch (monthly schedule reminders, retention nudges, broadcast campaigns) MUST default to disabled and require an explicit per-tenant `enabled` flag in `tenant_settings.settings_json` before the system will dispatch. The script + endpoint return a `skipped_disabled` summary when `enabled=false`. Transactional sends triggered by an explicit customer action (booking confirmation, payment receipt, single-shot operator broadcast) are exempt and continue to send. | [`chess-phase-4-1-redesign-care-list-2026-05-01.md`](../qa/chess-phase-4-1-redesign-care-list-2026-05-01.md) | 21 |
| `FR-107` | **Care-list opt-in mechanism**: recurring customer dispatches MUST be filtered to a tenant-curated care list. Membership lives on `contacts.metadata_json->>'care_list_member' = 'true'`, mutated only by tenant-admin via `POST /api/v1/tenants/me/care-list` + `DELETE /api/v1/tenants/me/care-list/{contact_id}`. Every mutation logs `outbox_events` with `event_type='tenant.care_list.member_added' / .member_removed`. The dispatch endpoint defaults `care_list_only=true`; override only via explicit operator force flag. | [`chess-phase-4-1-redesign-care-list-2026-05-01.md`](../qa/chess-phase-4-1-redesign-care-list-2026-05-01.md) | 21 |
| `FR-108` | **24h dispatch idempotency**: any recurring customer-comms dispatch MUST persist its last-run timestamp under `tenant_settings.settings_json.<feature>.last_run_at` and refuse to re-run within 24h unless an explicit `force=true` operator override is passed. Idempotency keys are namespaced per feature so chess monthly reminder vs aimentor retention vs futureswim newsletter do not block each other. | [`chess-phase-4-1-redesign-care-list-2026-05-01.md`](../qa/chess-phase-4-1-redesign-care-list-2026-05-01.md) | 21 |
...
| `NFR-053` | Compliance | Recurring customer-comms dispatches MUST be opt-in: default-off tenant flag + per-contact care-list membership. No tenant rollout that auto-enrols all contacts is allowed; UI MUST require a deliberate add per contact. Audit row in `outbox_events` for every membership mutation + every dispatch. Implements FR-106/FR-107/FR-108. | [`chess-phase-4-1-redesign-care-list-2026-05-01.md`](../qa/chess-phase-4-1-redesign-care-list-2026-05-01.md) | 21 |
| `NFR-054` | Governance | **No surprise automation**: any background job that auto-executes (OS cron, n8n cron, Claude Code `/schedule` routine, `RemoteTrigger` cloud agent, scheduled lifecycle worker, retention cadence, soak audit, etc.) MUST be (a) explicitly approved in writing for the specific scope + cadence, (b) listed in a single registry doc with owner + cadence + kill-switch, and (c) reversible from the tenant or operator UI without a deploy. New automation defaults to **disabled** until the registry entry + kill-switch ship. Operator-triggered ("Send now" button, manual CLI dry-run) is preferred over scheduled execution wherever the volume allows. | operator directive 2026-05-01 | 21, 23 |
| `NFR-060` | Observability | Prometheus + Grafana + AlertManager with operator-approved alert routing; Discord is not a default route and is enabled only when explicitly requested for a task or incident lane | full-stack review + operator preference 2026-05-01 | 20, 23 |
...
- `2026-05-01` add FR-106/FR-107/FR-108 + NFR-053 codifying default-off + per-contact opt-in for recurring customer-comms dispatches. Source: chess.bookedai.au Phase 4 §1 redesign — operator feedback rejected the original blast-everyone monthly reminder. Pattern is now reusable across futureswim/aimentor/future verticals.
- `2026-05-01` add NFR-054 — "no surprise automation". Any background job (OS cron, n8n cron, /schedule routine, RemoteTrigger agent, scheduled worker) requires written approval, registry entry, kill-switch, and defaults to disabled. Manual operator triggers are preferred. Source: operator directive after the no-cron + no-/schedule sequence on 2026-05-01.
- `2026-05-01` update NFR-060 and closeout posture — Discord is no longer a default documentation, closeout, or alerting surface. Use repo docs, memory, and Notion/archive by default; enable Discord only when explicitly requested for the specific task or incident lane.
