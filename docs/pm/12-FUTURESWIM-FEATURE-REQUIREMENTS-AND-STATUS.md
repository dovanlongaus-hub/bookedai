...
| 13 | Calendar reminders | Partial | Phase 4 (this PR) — relies on generic tenant broadcast tool from chess Phase 4 branch (`feat/chess-phase-4-broadcast-monthly-2026-05-01`) | Coach can fire schedule reminders ad-hoc from tenant admin workspace; cron-driven T-24h / T-2h cadence still needs the futureswim default `reminder_cadence` to be set | Manual broadcast wired now; automated cadence still 0.5d BE follow-up. See Deferred → 4.1 |
...
| 15 | Monthly notification | Live | Phase 4 (this PR) — migration 055 + generalized `monthly_reminder_service` from chess Phase 4 branch | `tenant_settings.settings_json.monthly_reminder.enabled = true` for tenant `future-swim`; the generalized worker dispatches monthly digest to opted-in care-list members on next run | Care-list opt-in is per-contact via `contacts.metadata_json.care_list_member`. Default off until parent opts in via portal/WhatsApp |
| 16 | Mass notification to parents by area (general info) | Live | Phase 4 (this PR) — migration 054 + generic tenant broadcast endpoint | Coach/admin opens `tenant.bookedai.au/?tenant=future-swim` → Broadcasts → audience=region → picks `caringbah` (or any other centre). Migration 054 backfilled `contacts.metadata_json.region` from `futureswim_parent_users.centre_code` so the audience filter resolves to real parents | Linked from `/coach` footer ("Open tenant admin → Broadcasts"). Real send still requires a tenant admin session (see Deferred → 4.1 onboarding task) |
| 17 | Individual notification per student | Live | Phase 4 (this PR) — generic tenant broadcast endpoint with audience=student | Same tenant admin workspace exposes audience=student; coach selects an individual parent + composes channel-specific message via shared `CommunicationService` | Wired generically in tenant tool; per-student send needs tenant admin session (Deferred → 4.1) |
...
| 053 | `future_swim_demo_parent_bookings.sql` — seeds 2 demo `booking_intents` rows for the demo parent so the parent portal "your bookings" timeline has live data |
| 054 | `future_swim_contacts_region_backfill.sql` — backfills `contacts.metadata_json.region` from `futureswim_parent_users.centre_code` so the generic tenant broadcast tool's audience=region filter resolves to real parents (Phase 4) |
| 055 | `future_swim_monthly_reminder_enable.sql` — enables `tenant_settings.settings_json.monthly_reminder.enabled = true` for tenant `future-swim` so the generalized monthly_reminder_service starts dispatching to opted-in care-list members (Phase 4) |
...
| Tenant admin onboarding for futureswim staff (real Future Swim email + magic-link) | 4.1 | 0.5d BE+ops | Generic broadcast tool is wired and migration 054 + 055 are live, but no tenant admin row exists yet for futureswim. Once a Future Swim staff email is provided, seed `tenant_admin_users` + send first magic-link to unlock the live `tenant.bookedai.au` workspace |
| Calendar reminders automated cadence (T-24h + T-2h) | 4.1 | 0.5d BE | Set futureswim default `reminder_cadence`; reuse existing `reminder_next_at` cron worker. Manual broadcast already works in Phase 4 |
...
- `2026-05-01` Phase 4 update: requirements #13 → Partial, #15 → Live, #16 → Live, #17 → Live. Adds migrations 054 (contacts region backfill) + 055 (monthly reminder enable). Coach dashboard footer now links to the generic tenant broadcast workspace at `tenant.bookedai.au/?tenant=future-swim`. Outstanding: tenant admin onboarding for real Future Swim staff (Phase 4.1) + automated calendar reminder cadence (Phase 4.1).
