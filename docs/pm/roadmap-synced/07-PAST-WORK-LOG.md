...
- **AI Mentor 1-1 tenant seeded `2026-04-21`; contact/login refreshed `2026-04-28`** — slug `ai-mentor-doer`, tagline `Convert AI to your DOER`, 10 catalog packages (5 private 1-1 + 5 group mentoring), USD currency, runtime `https://ai.longcare.au/`, embed via `https://product.bookedai.au/partner/ai-mentor-pro/embed?embed=1&tenant_ref=ai-mentor-doer`, plugin loader `/partner-plugins/ai-mentor-pro-widget.js`, login `aimentor@bookedai.au / FirstHundred1M$`, contact email `aimentor@bookedai.au`, phone/WhatsApp/Telegram/iMessage `+84908444095`; seed migration `013_ai_mentor_tenant_seed.sql` plus migration `023_ai_mentor_contact_login_update.sql`; plugin interface doc [`docs/development/ai-mentor-pro-plugin-interface.md`](../../development/ai-mentor-pro-plugin-interface.md); custom asset pack at [`frontend/public/tenant-assets/ai-mentor/`](../../../frontend/public/tenant-assets/ai-mentor/)
...
- **AI Mentor 1-1 production embed shell verified `2026-04-28`** — live embed URL returned `200`, loader `/partner-plugins/ai-mentor-pro-widget.js` returned `200` with immutable cache headers, and `embed=1&tenant_ref=ai-mentor-doer` is deployed on `product.bookedai.au`; full D-1 rehearsal still needs browser-level catalog/booking proof, and static `OPTIONS` currently returns `405` rather than the target CORS `204`.
...
| AI Mentor 1-1 demo runnable | MEDIUM | Tenant seeded `2026-04-21` via migration `013`; contact/login refreshed and applied `2026-04-28`; 10-package catalog live; runtime `https://ai.longcare.au/`; production embed shell and loader verified `200` after deploy `3f8bcb9`; full browser-level catalog/booking rehearsal remains for M-01 D-1 `11:30`, and CORS `OPTIONS` still returns `405` instead of target `204` |
