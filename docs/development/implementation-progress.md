...
- published the closeout to Notion with Discord skipped; archive entry: `docs/development/telegram-sync/2026-05-01/141431-requirement-docs-closeout-posture-sync-2026-05-01.md`
- verification is documentation-only with `git diff --check`
...
- update: re-split the two public surfaces by audience. `frontend/src/apps/public/PublicApp.tsx` now defaults to an SME buying message (`Get a booking-ready sales page for your service business.`) with CTAs around seeing a live booking flow and getting the booking page set up.
- update: removed weak/internal homepage wording from the default SME path, including `payment posture`, `tenant proof`, `action runs`, `judge and owner`, and investor-first proof language. The page now emphasizes the done-for-you setup: page, inbox, CRM, calendar, meeting links, payment next step, and follow-up.
- update: reframed `frontend/src/apps/public/PitchDeckApp.tsx` back toward investor/judge positioning. The hero now says `BookedAI is the AI Revenue Engine for service businesses.`, with live booking proof, investor deck navigation, system map, SME pilot, and repeatable deployment package language.
- update: fixed the product booking follow-up tenant-session failure by adding public-safe tenant fallback resolution in `backend/api/v1_communication_handlers.py` for public-web and embedded-widget communication automation. Public booking actors now resolve to the `bookedai-au` tenant when the strict tenant-session resolver returns the public endpoint guidance.
- update: published the closeout to Notion and Discord; archive entry is `docs/development/telegram-sync/2026-04-29/032546-homepage-pitch-audience-split-product-tenant-fix.md`.
- verification: `python3 -m py_compile backend/api/v1_communication_handlers.py`; `.venv/bin/python -m pytest backend/tests/test_api_v1_communication_routes.py -q` (`6 passed`); `npm --prefix frontend run build`; `cd frontend && npx playwright test tests/public-homepage-responsive.spec.ts tests/pitch-deck-rendering.spec.ts tests/pitch-architecture-viz.spec.ts --project=legacy` (`8 passed`); `git diff --check`.

## 2026-04-29 Homepage Chat Search Failure Recovery

- update: fixed the homepage chat-search failure state in `frontend/src/apps/public/HomepageSearchExperience.tsx`. If live search, matching, or legacy chat endpoints fail after a visitor searches, the assistant now keeps the booking flow open, shows fast preview/catalog fallback results, and uses customer-safe recovery copy instead of exposing raw loading/backend failure text.
- update: added Playwright coverage in `frontend/tests/public-homepage-responsive.spec.ts` that forces the relevant search/chat endpoints to return `500` and verifies the homepage shows the recovery message, does not show `Search needs attention`, and does not leak simulated backend errors.
