# Email sender identity aligned on info@bookedai.au

- Timestamp: 2026-04-27T09:28:54.106902+00:00
...
BookedAI-managed outgoing email identity is now configured to use info@bookedai.au across runtime env, backend defaults, Docker compose, Supabase email settings, webapp placeholders, and live backend/beta-backend containers. Deploy and stack health passed. A follow-up SMTP auth check returned `SMTPAuthenticationError`, so real email delivery still needs valid Zoho app-password secrets for `info@bookedai.au`.
