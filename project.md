...
- the latest pitch video at `https://upload.bookedai.au/videos/0cfb/LCpooAUVSsL24QXMvIBR0A.mp4` is now embedded with native controls on both `bookedai.au` and `pitch.bookedai.au`, giving visitors an early watchable overview before they continue into live product proof or the deeper deck
...
- Sprint 19 P0-8 hardening changes the repo default: OpenClaw CLI is rootless, no hostfs/Docker-socket mount is declared by default, `host_shell`/`openclaw_runtime_admin`/`full_project` are removed from the default Telegram action vocabulary, and `host-shell` is blocked unless `BOOKEDAI_ENABLE_HOST_SHELL=1` is explicitly set for break-glass work
