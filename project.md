...
- `2026-04-29` tenant workspace enterprise shell redesign is implemented locally for `tenant.bookedai.au`: the tenant runtime now has a sticky enterprise top bar, icon action controls, a 2xl desktop icon rail, sticky left workspace menu, right-side content frame, contextual panel actions, and mobile bottom icon navigation while preserving the existing tenant read models and panel logic
- tenant UAT/A-B coverage now includes a mocked workspace path for `future-swim` that exercises desktop and mobile navigation, command palette opening, random panel switching across all tenant panels, control/revenue-ops variant stability, screenshots, and no-horizontal-overflow assertions; verification passed with TypeScript no-emit, frontend build, tenant smoke (`6 passed`), and `frontend/tests/tenant-gateway.spec.ts` (`6 passed`)
...
- mobile and Android after-booking UX is now guarded by regression coverage so wallet/order actions stay contained on `390px` and `412px` widths while desktop keeps a broader order-summary layout; frontend production build and product regression passed with `8` Playwright tests
- local `product.bookedai.au` UI/auth/search UX review pass is implemented: the top bar now shows only the logo mark without the redundant adjacent `BookedAI` text, adds a compact account menu with Google Identity Services support when `VITE_GOOGLE_CLIENT_ID` is configured plus an email/name fallback, stores the customer profile locally, and pre-fills later booking details so returning customers are not asked for the same name/email/phone again unless they edit the fields
- the product assistant journey now uses a clearer six-state booking model (`Chat`, `Search`, `Preview`, `Book`, `Pay`, `Care`) on larger screens while hiding that strip on mobile to preserve the search/chat-first workspace; booking steps are status-aware and the selected booking form shows when saved contact details are being reused
- slow live-read search now has stronger perceived-performance handling: tenant/catalog matches can appear immediately, service shortcuts understand tenant aliases such as chess, swim, mentor, WSTI, and event, live-read ranking is bounded by a timeout, and the staged progress copy tells customers when tenant matches are shown while deeper ranking catches up
- verification passed locally with `cd frontend && npx vite build`, product Playwright regression (`7 passed`), and responsive browser sweeps at `390x844`, `412x915`, and `1440x1000` showing `0` horizontal overflow and no console/request errors; the full `npm --prefix frontend run build` remains blocked by unrelated `frontend/src/apps/public/ChessGrandmasterApp.tsx` TypeScript errors
- Zoho CRM/Calendar lifecycle setup is activated live for BookedAI booking operations: the Zoho OAuth helper now requests CRM, notification, Calendar event, Calendar list, and Zoho Meeting scopes in one consent flow; `list-calendars` selected the AU `info` calendar UID; booking meeting provisioning reads `ZOHO_CALENDAR_API_BASE_URL=https://calendar.zoho.com.au/api/v1`, sends Zoho `eventdata`, and refreshes Calendar access tokens at runtime; and paid-booking reconciliation attempts immediate Zoho `Deal` Closed Won plus follow-up `Task` sync with payment amount/source/time while retaining local audit/outbox/email truth
- current live UAT confirms the AU Zoho path end to end: booking `v1-364c66e949` returned both a Zoho Meeting URL and Calendar event URL, CRM booking deal/task synced, authenticated manual payment confirmation marked the booking paid, and Zoho payment deal/task records synced after one provider throttle retry
- local product mobile shell polish is implemented for `product.bookedai.au`: the oversized first-screen explainer copy was reduced from the long `Chat, search, preview, booking, payment posture, calendar, CRM/email, and customer-care follow-up...` sentence to a compact flow label, the redundant free-trial sentence was removed from the mobile viewport, the flow steps now render as compact horizontal chips, and the assistant container gets the freed space so the chat remains the primary surface; verification passed with frontend build and `frontend/tests/product-app-regression.spec.ts` (`6 passed`)
- local Stripe best-practice integration is implemented across direct backend Checkout/Billing REST calls: API version pinning now lives in `backend/integrations/stripe/constants.py` (`2026-02-25.clover`), Checkout Session creation no longer hardcodes card-only payment methods, `scripts/stripe_sandbox_smoke.py` gives operators a safe test-key-only sandbox smoke path, and homepage Stripe return now verifies `GET /api/v1/payments/status` before showing paid; Playwright now covers `verifying -> paid`, and `scripts/stripe_webhook_replay_smoke.sh` documents the Stripe CLI replay path, currently blocked locally because `stripe` is not installed; detailed notes: `docs/development/stripe-best-practices-integration.md`, `docs/development/stripe-payment-flow-review-2026-04-28.md`, and `docs/development/stripe-cli-webhook-replay-runbook.md`; Notion/Discord archive entries: `docs/development/telegram-sync/2026-04-28/154445-bookedai-stripe-best-practices-integration.md`, `docs/development/telegram-sync/2026-04-28/155756-bookedai-stripe-payment-flow-review-and-verified-return-follow-up.md`, and `docs/development/telegram-sync/2026-04-28/161310-bookedai-stripe-return-e2e-and-webhook-replay-runbook.md`
- local Phase 21 payment reconciliation foundation is implemented: Stripe paid webhooks now call shared `PaymentLifecycleService.mark_booking_paid(...)`, tenant admin/finance users have an authenticated `POST /api/v1/payments/manual-confirm` path for received QR/bank/manual payments, and `GET /api/v1/payments/status` exposes backend paid/verifying truth for portal/public return surfaces; backend regression across booking, Stripe, search, messaging, and Zoho feedback returned `139 passed`, with Notion/Discord archive `docs/development/telegram-sync/2026-04-28/152225-bookedai-payment-reconciliation-lifecycle-local-implementation.md`
- customer communication templates now carry the current BookedAI.au brand system across outbound lifecycle surfaces: confirmation emails use the official logo, dark brand header, status badge, structured booking summary, portal/CTA link, and `info@bookedai.au` support footer; WhatsApp booking confirmations use native short-line `*bold*` sections; Telegram search/order/booking replies add compact HTML-safe status/action sections while keeping inline controls
- `bookedai-au` public booking confirmations now also send a compact carrier SMS via Twilio when the customer supplies a phone number, so the public funnel is not dependent on WhatsApp app availability for the first booking receipt
- local verification for this communication-template polish passed with focused backend regression across lifecycle email rendering, release-gate security fixtures, booking routes, Telegram webhook routes, and WhatsApp webhook routes; Notion/Discord archive entry is `docs/development/telegram-sync/2026-04-28/143344-bookedai-communication-template-polish.md`
...
- the tenant-branded production host is live at `https://aimentor.bookedai.au`, routed through the shared frontend/backend runtime with Cloudflare DNS automation, Nginx/TLS deployment, and CORS/TrustedHost defaults aligned to tenant `ai-mentor-doer`; live smoke passed for homepage, account route, backend health, certificate SAN, stack health, and app-bundle routing
...
- AI model routing now defaults customer-facing backend calls to OpenAI (`OPENAI_API_KEY`, `OPENAI_MODEL=gpt-5-mini`) and can fall back to Claude through Anthropic auth (`ANTHROPIC_API_KEY`, `ANTHROPIC_MODEL`) when configured; OpenAI remains required for internet/public-web search because the service uses the Responses `web_search` tool and labels sourced options as `public_web_search`
...
- latest live Telegram reactivation on `2026-04-27` confirmed both bot surfaces: operator `@Bookedairevenuebot` webhook points to `https://api.bookedai.au/telegram-webhook` with pending `0` and no last error, while customer `@BookedAI_Manager_Bot` webhook points to `https://api.bookedai.au/api/webhooks/bookedai-telegram` with pending `0`; tenant `co-mai-hung-chess-class` now has Telegram tenant notification chat id `8426853622`
...
- Telegram customer replies should use Telegram-native presentation: HTML-safe rich text for headings, labels, links, and short sections, plus inline keyboard actions for the next step. Plain text fallback remains acceptable for provider/client failure cases, but normal `BookedAI Manager Bot` service search, existing-order confirmation, pending-payment booking, and booking handoff messages should render as compact rich Telegram cards with clear actions.
...
- Telegram chat can now capture the first direct booking intent from the shortlist: after a service-search reply, the customer can answer `Book 1` with name plus email or phone and preferred time; BookedAI creates a real `booking_intents` record, contact, lead, booking reference, portal link, and pending payment/follow-up state, then returns a rich Telegram menu to keep the booking, view the portal/QR, request a time change, cancel, or start a new booking search
...
Latest customer-care/admin follow-up from `2026-04-27`:

- `product.bookedai.au` and the landing assistant now expose a first-class Telegram care handoff to `@BookedAI_Manager_Bot` before and after booking: pre-booking links can carry the selected service context, and post-booking links/deep links carry the booking reference so returning support lands in the correct customer-care thread
- the backend Telegram webhook now rewrites `/start bk.<booking_reference>` into booking-aware customer-care context, preserving portal-grounded answers instead of treating the deep link as a generic new chat start
- Telegram booking-intent lifecycle metadata is now preserved safely when multiple side effects happen in one turn: `lifecycle_updates` no longer gets overwritten by later `queued_request` merges inside `backend/api/route_handlers.py`
- Telegram existing-booking payment/status care now classifies support escalation from the latest customer message only, preventing prior assistant history such as `Support contact` from accidentally queuing a support request; the same replies now include a richer Telegram action menu for keeping the booking, viewing portal/QR, changing time, cancelling, or starting a new booking search
- portal booking continuity now mirrors that handoff on the web surface: loaded `portal.bookedai.au` bookings show booking QR, payment QR/posture, copy/save/download controls, Telegram and WhatsApp continuation links with the booking ID attached, and clear next-effect guidance for pay, reschedule/change, cancel, help, and add-new-booking actions
- admin reliability/integrations now surfaces protected customer-agent health plus tenant-scoped spotlight signals for Telegram reply state, callback acknowledgement, webhook backlog, CRM sync, email confirmation, and notify/outbox posture so operators can see whether the customer-care loop actually completed, queued, or needs manual review
- Phase 19 now also carries a regression/UAT requirement for the customer Telegram booking path: Book n intent, CRM/email side effects, deep-link reopen, and tenant proof for `chess` must stay provable before Phase 20 widget expansion
...
- `2026-04-27` runtime email configuration follow-up: root `.env` now uses `info@bookedai.au` for `BOOKING_BUSINESS_EMAIL`, `EMAIL_SMTP_USERNAME`, `EMAIL_SMTP_FROM`, and `EMAIL_IMAP_USERNAME`; Supabase email admin/certbot settings use the same mailbox; Docker dev/prod compose now falls back to `info@bookedai.au` for SMTP username, From, and IMAP username; backend settings treat blank email env values as missing so customers and partners do not receive BookedAI-managed emails from tenant-specific mailboxes.
...
- Sprint 19 P0-5 is closed live: public assistant v1 routes reject mismatched `actor_context.tenant_id` values with `403` when a tenant Bearer session is authenticated, and the live tenant mismatch smoke passed on `2026-04-26`
- Sprint 19 P0-4 is partly live: webhook idempotency routing and evidence indexes are in code/live DB for WhatsApp, customer Telegram, and Evolution; Telegram customer UAT still needs a real UAT chat id and the operator evidence drawer remains a UI follow-up
- Phase 17 P1-9 is closed live: Future Swim Miranda now points its BookedAI catalog booking URL at `https://futureswim.com.au/locations/` instead of the dead `/locations/miranda/` branch page
- Phase 19 P1-3 is closed locally: WhatsApp webhook tests now cover identity-gate, queued cancel, queued reschedule, and Internet expansion at parity with Telegram
- Phase 19 FX-2 has its shared-client first pass closed live: typed frontend API calls through `frontend/src/shared/api/client.ts` now use a 30-second timeout, while direct `fetch` cleanup remains a carried reliability task
- Phase 17 FX-7 is closed live: `portal.bookedai.au` accepts `booking_reference`, `bookingReference`, `ref`, and hash booking-reference links, resolves the first non-empty reference, warns on conflicts, and rewrites the browser URL to a single canonical `booking_reference` param after load
...
- Latest operator instruction reverses the Sprint 19 P0-8 rootless default for the operator surface: OpenClaw CLI now defaults to root/full-host mode with privileged host PID access, `/hostfs`, Docker socket access, `BOOKEDAI_ENABLE_HOST_SHELL=1`, and trusted Telegram actions including `host_shell`, `openclaw_runtime_admin`, and `full_project`
- live OpenClaw v3 was recreated with the full-host default on `2026-04-27`; `openclaw-bookedai-cli` verified as root, privileged, host PID, `/hostfs` and Docker socket mounted, with gateway and CLI health green
...
- `2026-04-27` Product full-flow UI/UX review merged the duplicated `search`, `shortlist`, `match`, `review`, and `follow-up` wording into one customer journey for `product.bookedai.au`: `Chat request -> Search -> Preview -> Select -> Book -> Payment posture -> Thank You / Portal -> Add calendar -> CRM/email/messaging -> Telegram/customer-care handoff`
- the Product page flow rail now reads `Chat / Search / Preview / Book / Pay / Care`, and the assistant confirmation state now shows a dedicated `Customer care handoff` plus a `BookedAI Manager Bot / portal` preview card anchored to the booking reference
- the Product assistant welcome shortcuts now prioritize the current proof paths: `Chess`, `Future Swim`, `AI Event WSTI`, and `AI Mentor 1-1`, with all four available on compact mobile
- Product copy now treats payment, calendar, CRM/email, messaging, and Telegram care as status-aware handoffs, avoiding claims that lifecycle actions completed instantly when the backend only reports pending, queued, or ready states
- the post-booking Thank You state no longer duplicates the booking QR: the portal hero owns the single QR, while the lower area is a booking summary plus portal-action panel
- live-read Product popup cards now preserve tenant/public-web provenance, `Book online` posture, and next-step/callback guidance even if chat-copy streaming falls back
- slow search feedback and performance were tightened: Product now shows instant local/catalog matches while live-read ranking verifies availability, maps, partner links, and Internet results; duplicate shadow search is skipped once live-read has grounded the result
- the booking-to-thank-you flow now includes compact portal review/edit/reschedule/cancel/Telegram care decision cards; confirmation email previews state `From: info@bookedai.au`
- linked Telegram follow-up is available after booking through `@BookedAI_Manager_Bot`: the backend sends only when a phone is linked to an existing Telegram chat session, otherwise it records a queued/manual-link state because Telegram bots cannot initiate a chat from phone alone
- verification passed with frontend TypeScript, `frontend/tests/product-app-regression.spec.ts` (`6 passed`), live-read Product popup UAT (`4 passed`), frontend build, backend communication+booking route tests (`12 passed`), and the release gate with frontend smoke/tenant/backend/search eval all passing; live promotion completed with `python3 scripts/telegram_workspace_ops.py deploy-live`, stack health at `2026-04-27T02:45:45Z`, `product.bookedai.au` returning `HTTP/2 200`, live bundle checks for the new slow-search/Telegram/email/shortcut copy, and mobile `390x844` smoke confirming the Product assistant shortcuts with no horizontal overflow
...
- `deploy-live` now treats host users `openclaw` and `telegram` as trusted full deploy execution contexts: the operator command invokes the VPS host-shell path from `/home/dovanlong/BookedAI`, and the `deploy_live_host.sh` wrapper can self-enter the host namespace through `nsenter` when called directly from a privileged OpenClaw runtime with `/hostfs`
- `host-exec` is now an explicit alias for the full `host-shell` execution path so OpenClaw/Telegram can call `python3 scripts/telegram_workspace_ops.py host-exec --cwd / --command "sudo -n ..."` and receive the same host namespace plus sudo-capable behavior as `host-shell`
- the host-shell execution helper now skips `sudo` whenever `nsenter` host context is available, so slim OpenClaw runtimes with `/hostfs` but no `sudo` binary can still enter the VPS namespace and run the live deploy path
- the host-shell helper also supports the inverse runtime shape: when a non-root runtime sees `nsenter` but cannot execute it directly, it falls back to `sudo -n nsenter ...`; after entering the VPS namespace, commands may use host `sudo`, so trusted OpenClaw/Telegram sessions can run full host repair and deploy commands under the existing `openclaw`/`telegram` sudoers posture
- OpenClaw runtime-admin repair actions use the `openclaw_runtime_admin` permission scope, including the exec approval policy fix and full-access runtime apply command; this scope is now granted by default to trusted Telegram operators
- `openclaw_runtime_admin` also gates the full-access OpenClaw runtime repair command, and the trusted Telegram/OpenClaw default now includes full-host `host-shell` access for operator-scoped VPS deploy and repair work
...
## 2026-04-28 BookedAI.au Public Chat Tenant Lead Routing

The public homepage chat is now aligned with the tenant-scoped public lead API:

- `bookedai.au` public assistant booking submit routes public-web and embedded-widget lead capture to `/api/v1/public/leads/bookedai-au`
