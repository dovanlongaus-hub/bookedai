  ...
  return (
    <AdminCard className="p-6 xl:col-span-2">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-950">Unified investigation timeline</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600">
            This feed merges tenant auth, billing, CRM, integration, and support-session signals into one read-only case view so operators can reason about tenant issues in sequence instead of hopping across separate cards.
          </p>
        </div>
      ...
      <div className="mt-5 space-y-3">
        {items.length > 0 ? (
          ...
          <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-600">
            No investigation signals are available yet for this tenant.
          </div>
        )}
      </div>
    </AdminCard>
