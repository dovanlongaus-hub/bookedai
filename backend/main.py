...
async def lifespan(app: FastAPI):
    ...
    app.state.booking_assistant_service = BookingAssistantService(settings)
...
async def upload_image(
    ...
    relative_dir = Path("images") / secrets.token_hex(2)
    ...
    return {
        "filename": filename,
        "content_type": file.content_type or f"image/{extension.removeprefix('.')}",
        "size": len(content),
...
async def admin_booking_confirm_email(
    ...
    lines.extend(["", f"Reply to {request.app.state.settings.booking_business_email} if you need help."])
...
async def admin_config(
    ...
    raw_items: list[tuple[str, str, str, bool]] = [
        ("OPENAI_API_KEY", cfg.openai_api_key, "OpenAI", True),
        ("OPENAI_MODEL", cfg.openai_model, "OpenAI", False),
